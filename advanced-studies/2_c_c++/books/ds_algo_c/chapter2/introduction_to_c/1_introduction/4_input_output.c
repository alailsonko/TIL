...
In this tutorial, you will learn to use scanf()
function to take input from the user, and printf()
...
Output > Number = 5
We use %d format specifier to print int types. Here,
...
Output > number 1 = 13.5000000000
Output > number 2 = 12.4000000000

To print float, we use %f format specifier. Similarly, we use %lf
...
to take input from the user. The scanf() function reads formatted
...
function to take int input from the user. When the user enters an integer,
it is stored in the testInteger variable.

|Note: Notice, that we have used &testInteger inside scanf().
...
Output > Enter a number: 12.424
Output > Enter another number: 12.4
Output > num1 = 12.4240000
Output > Num2: 12.400000

We use %f and %lf format specifier for float and double
respectively.
```
...
Output > You entered g

When a character is entered by the user in the above program,
the character itself is not stored. Instead, an integer value (ASCII value)
is stored.

And when we display that value using %c text format, the entered
character is displayed. If we us %d to display the character, its ASCII
value is printed.
```
...
output > Enter a character: g
output > You entered g
output > ASCII value is 103.
```
## I/O Multiple Values

Heres how you can take multiple inputs from the user and displays
them.
```
#include <stdio.h>
int main() {
...
output > Enter integer and then a float: -3
output > 3.4
output > You entered -3 and 3.400000
```
